...
`git clone https://github.com/ByteLegendQuest/java-fix-missing-default-constructor`

A project directory named `java-fix-missing-default-constructor` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-missing-default-constructor` directory which is created just now.
